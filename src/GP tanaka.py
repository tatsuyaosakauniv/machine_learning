...
def train_step(train_sample,latent_input):
    ...
    for l in range(d_steps_add):
        with tf.GradientTape() as tape :
            ...
            gp =  GP_tanaka(data,reconstruction_sample,batch_size)  # Gradient Pnealty

            d_loss = d_cost + gp*gp_weight                              # Wasserstein loss + GP
    ...
    with tf.GradientTape() as tape :
        ...
        gp =  GP_tanaka_G(data,reconstruction_sample,batch_size)  # Gradient Pnealty G

        gz_loss = gz_cost +  gp*gp_weight   #Wasserstein Loss +GP．（一般的には生成器にGPを与えないが，自分はつけている．）
    ...
    with tf.GradientTape() as tape :
        ...
        g_loss = g_cost +gp*gp_weight  #Wasserstein Loss のみ．（一般的には生成器にGPを与えないが，自分はつけている）
