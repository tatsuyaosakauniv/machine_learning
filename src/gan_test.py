...
c_noise = tf.keras.layers.Dense(
    units=32, activation=tf.keras.layers.LeakyReLU(alpha=0), 
    kernel_initializer="he_normal"
)(noise_inputs)
innx = tf.keras.layers.Dense(sequence_length, activation=tf.keras.layers.LeakyReLU(alpha=0),kernel_initializer="he_normal")(c_noise)  # Flatten 削除済み
nxout = tf.keras.layers.Dense(sequence_length, activation=tf.keras.layers.LeakyReLU(alpha=0),kernel_initializer="he_normal")(innx)

#-- hidden layers
x1 = tf.keras.layers.Dense(hidden_node, activation = tf.keras.layers.LeakyReLU(alpha = 0),kernel_initializer = "he_normal")(nxout)
...
#-- output
decoded = tf.keras.layers.Dense(units = sequence_length)(x3)
...
# 保存
plt.savefig(r"/home/kawaguchi/result/" + result_dir + "/heatflux_pred.svg", dpi=600, bbox_inches='tight')
plt.close()
...
for x_min, x_max in x_ranges:
    ...
    plt.tight_layout()
    plt.savefig(f"/home/kawaguchi/result/" + result_dir + "/heatflux_pred_{x_min}_{x_max}.svg", dpi=600, bbox_inches='tight')
    plt.close()
...
plt.savefig(r"/home/kawaguchi/result/" + result_dir + "/ACF_pred.svg", dpi=600, bbox_inches='tight')
plt.close()
...
# プロットを保存
plt.savefig(r"/home/kawaguchi/result/" + result_dir + "/ITR_pred.svg", dpi=600, bbox_inches='tight')
plt.close()
...
# # プロットを保存
# plt.savefig(r"/home/kawaguchi/result/" + result_dir + "/ITC_pred.svg", dpi=600, bbox_inches='tight')
# plt.close()
